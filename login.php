<?php
	...
	*/
	...
	if( !empty( $_POST ) ) {
		...
		//Je relie mes fichiers qui contiennent toutes mes fonctions (account_exist et ident_libre)
		require_once ( 'incl/fonctions/fonct_connexion.php' );	
		...
		*/
		if( $membre ) {
			...
			//On me redirige sur 'index.php' car je n'ai rien à faire dans connexion si je suis déjà connecté
			header( "Location: index.php" );
			//echo '<script>window.location.replace("index.php")</script>';
		}else {
	...
	<body>
		<div class="container-fluid">
			<div id="formConnexion" class="row">	
				<div id="form1">
					<form id="form" class="form-horyzontal" role="form" action="login.php" method="POST">
					
						<h1>Connexion</h1>
						
						<!-- Si tableau assoc. login non vide alors affichage erreur -->
						<?php if( isset($erreur['login']) ) :?>
							<div id="erreurEmail" class="btn btn-danger"><?= $erreur['login'] ?></div>
						<?php endif; ?><br/>
						
						<!-- Si tableau assoc. mdp non vide alors affichage erreur -->
						<?php if( isset($erreur['mdp']) ) :?>
							<div id="erreurEmail" class="btn btn-danger"><?= $erreur['mdp'] ?></div>
						<?php endif; ?><br/>
						
						<!-- Si tableau assoc. membre non vide alors affichage erreur -->
						<?php if( isset($erreur['membre']) ) :?>
							<div id="erreurEmail" class="btn btn-danger"><?= $erreur['membre'] ?></div>
						<?php endif; ?><br/>
						
						<!-- Si tableau assoc. validation non vide alors affichage erreur -->
						...
						<div class="input-group margin-bottom-sm">
							...
							<!-- Sauvegarde du login pour éviter de le recopier -->
